...
_XML_PATH = _HERE / "xmls" / "arena_arm_hand_monitor_mouse.xml"
...
class PandaClickMouseGymEnv(MujocoGymEnv):
    ...
    def __init__(
        ...
        qpos_addrs = []
        for name in self._allegro_joint_names:
            jid = mujoco.mj_name2id(self._model, jt, name)
            if jid < 0:
                qpos_addrs.append(None)
            else:
                try:
                    addr = int(self._model.jnt_qposadr[jid])
                    qpos_addrs.append(addr)
                except Exception:
                    qpos_addrs.append(None)
        self._allegro_qpos_addrs = qpos_addrs

        self._allegro_default_pose = np.array(
            [
                0.4700,
                0.3333,
                -0.0015,
        ...
        try:
            self._display_material_id = self._model.material(
                "display_new-0_material-0"
            ).id
            self._display_rgba_default = self._model.mat_rgba[
                self._display_material_id
            ].copy()
        except Exception:
            pass
            # print("[Warning] failed to find display material for color change:", e)

        self._mousepad_geom_id = -1
        self._mousepad_radius = None
        try:
            self._mousepad_geom_id = self._model.geom("mousepad").id
            self._mousepad_radius = float(self._model.geom_size[self._mousepad_geom_id][0])
        except Exception:
            pass
            # print("[Warning] failed to read mousepad geom size:", e)
                ...
                "state": gym.spaces.Dict(
                    {
                        ...
                        "mouse_ori_pose": gym.spaces.Box(
                            -np.inf, np.inf, shape=(7,), dtype=np.float64
                        ),
                        "mousepad_ori_pose": gym.spaces.Box(
                            -np.inf, np.inf, shape=(7,), dtype=np.float64
                        ),
        ...
        front_id = self._get_cam_id_by_name("front")
        ego_left_id = self._get_cam_id_by_name("left")
        ego_right_id = self._get_cam_id_by_name("right")
        handcam_rgb_id = self._get_cam_id_by_name("handcam_rgb")
        ...
        self._mousepad_body_id = self._model.body("mousepad").id
        self._display_root_z0 = float(self._data.jnt("display_root").qpos[2])
        ...
        self._mouse_body_id = int(self._model.body("link_3").id)
        self._mouse_mass0 = float(self._model.body_mass[self._mouse_body_id])
        self._mouse_mass_mul = (0.75, 1.25)
    ...
    def _set_display_blue(self):
        if self._display_material_id < 0:
            return
        self._model.mat_rgba[self._display_material_id] = np.array(
    ...
    def _restore_display_color(self):
        if self._display_material_id < 0 or self._display_rgba_default is None:
            return
        self._model.mat_rgba[self._display_material_id] = self._display_rgba_default.copy()
        self._display_blue = False
    ...
    def reset(self, seed=None, **kwargs) -> Tuple[Dict[str, np.ndarray], Dict[str, Any]]:
        ...
        table_z = 0.92 + self.delta_h
        if self._table_site_id is not None:
            table_z = float(self._data.site_xpos[self._table_site_id][2])

        self._mouse_z = table_z + _MOUSEPAD_Z_OFFSET
        mouse_xy = np.float64(np.random.uniform(*_MOUSE_XY_BOUNDS))

        orig_quat = np.array(self._data.jnt("mouse_root").qpos[3:7], dtype=np.float64)
        yaw = np.deg2rad(np.random.uniform(*_YAW_PERTURB_BOUNDS))

        qw, qz = np.cos(yaw / 2.0), np.sin(yaw / 2.0)
        w1, x1, y1, z1 = qw, 0.0, 0.0, qz
        ...
        self.mouse_ori_pose = np.concatenate([mouse_xy, [self._mouse_z], q_new]).astype(
            np.float64
        )
        self._data.jnt("mouse_root").qpos = self.mouse_ori_pose

        monitor_xy = np.random.uniform(*_PLANT_XY_BOUNDS)
        try:
            display_qpos = self._data.jnt("display_root").qpos
            display_qpos[:3] = (
                monitor_xy[0],
                monitor_xy[1],
                self._display_root_z0 + self.delta_h,
            )
            display_qpos[3:7] = np.array([0.5, 0.5, -0.5, -0.5])
        except Exception:
            pass
            # print("[Warning] failed to set display_root pose:", e)

        try:
            # print("mousepad_body_id", self._mousepad_body_id)
            # print(
            #     "mousepad_pos",
            #     ([mouse_xy[0], mouse_xy[1], table_z]) + _MOUSEPAD_OFFSET,
            # )
            self._model.body_pos[self._mousepad_body_id][:3] = (
                np.array([mouse_xy[0], mouse_xy[1], table_z]) + _MOUSEPAD_OFFSET
            )
        except Exception:
            pass
            # print("[Warning] failed to set mousepad_root pose:", e)

        try:
            for addr, val in zip(self._allegro_qpos_addrs, self._allegro_default_pose):
                if addr is None:
                    continue
                self._data.qpos[addr] = float(val)
            mujoco.mj_forward(self._model, self._data)

            ctrl_ids = self._allegro_ctrl_ids
            valid_mask = ctrl_ids >= 0
            if np.any(valid_mask):
                self._data.ctrl[ctrl_ids[valid_mask].astype(int)] = self._allegro_default_pose[
                    valid_mask
                ]
        except Exception:
            pass
            # print("[Warning] failed to set Allegro initial qpos in reset():", e)
        ...
        # print("---original mouse mass---:", self._mouse_mass0)

        if self._randomize_dynamics:
            mul_mass = float(np.random.uniform(self._mouse_mass_mul[0], self._mouse_mass_mul[1]))
            # print(f"Randomizing mouse mass with multiplier: {mul_mass:.3f}")
            self._model.body_mass[self._mouse_body_id] = self._mouse_mass0 * mul_mass

            # print(
            #     "---dynamically randomized mouse mass---:"
            #     f"{self._model.body_mass[self._mouse_body_id]}"
            # )

        self._restore_display_color()
        try:
            self._mouse_joint0_init = _as_float(self._data.sensor("mouse_joint0_pos").data)
        except Exception:
            self._mouse_joint0_init = 0.0
    ...
    def _detect_click(self) -> bool:
        try:
            joint0 = _as_float(self._data.sensor("mouse_joint0_pos").data)
        except Exception:
            return False
    ...
    def _mouse_in_mousepad(self) -> bool:
        try:
            mouse_pos = self._data.sensor("mouse_pos").data
        except Exception:
            return False
        if self._mousepad_geom_id < 0 or self._mousepad_radius is None:
            return False
        pad_center = self._data.geom_xpos[self._mousepad_geom_id]
    ...
    def _compute_success(self) -> bool:
        # print("mouse_in_mousepad", self._mouse_in_mousepad())
    ...
    def _compute_observation(self) -> dict:
        ...
        obs = {
            "state": {
                ...
                "mouse_ori_pose": self.mouse_ori_pose,
...
if __name__ == "__main__":
    env = PandaClickMouseGymEnv(randomize=True, render_mode="human")
    env.reset()
    for _ in range(200):
